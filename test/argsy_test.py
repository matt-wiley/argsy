...
CLI_ARGS = dict(program=dict(name="argsy_test", description="cli_args_for_argsy_test"))


def test_parses_program_level_option_arg_with_input_value():
    test_args_input = CLI_ARGS
    test_args_input.get("program")["args"] = dict(
        thing=dict(cmd_type="option", flags="-t|--thing", help="Does a thing.")
    )
...
def test_parses_program_level_position_arg_with_input_value():
    test_args_input = CLI_ARGS
    test_args_input.get("program")["args"] = dict(
        thing=dict(cmd_type="position", help="Does a thing.")
    )
    argsy_parser = Argsy(config_dict=CLI_ARGS)
...
def test_parses_multiple_program_level_args():
    test_args_input = CLI_ARGS
    test_args_input.get("program")["args"] = dict(
        thing_a=dict(cmd_type="position", help="Does thing A."),
        thing_b=dict(cmd_type="option", flags="-b|--thing-b", help="Does thing B."),
        thing_c=dict(
            cmd_type="option",
            flags="-c|--thing-c",
            help="Does thing C.",
    ...
    parsed_args = argsy_parser.parse_args(
        ["a_value", "-b", "b_value", "-c"], print_result=True
    )
